...
**Auteur :** Brahimi Talla · matricule **257**
...
| **Relecteur** _(un étudiant désigné)_ | Recevoir l'assignation d'un exercice d'un pair, rendre une note entière 0–20 et un commentaire                                                | Relire son propre exercice ; modifier sa note après validation ; voir… _rien à voir : il ne voit que l'exercice à relire_ |
...
- **Authentification et gestion des comptes** — l'étudiant choisit son nom dans une liste, le formateur n'est pas authentifié (`Q1` : « Ne perdez pas de temps là-dessus »). _Conséquence assumée : toute personne ayant l'URL peut agir en tant que formateur ; c'est un exercice, pas une mise en production._
...
_Ce que tu exclus compte autant que ce que tu inclus. Un périmètre sans exclusion n'est pas un périmètre._
...
| **EF12** | Blocage après 5 codes erronés                     | Quand un étudiant se trompe 5 fois, la 6e tentative est refusée pendant 2 minutes (`Q4`) — _voir §7 : code `400 TROP_DE_TENTATIVES`, pour ne pas ajouter de code de statut à l'opération imposée_          | Could    |
...
| **`Q4` : aucun code HTTP prévu pour le blocage**                                                                                                                                                                      | `Q4`                                 | Réponse en **`400`** avec `code = TROP_DE_TENTATIVES` : on refuse d'ajouter un code de statut à une opération imposée (`B2`). Suivi en mémoire par étudiant (`TentativePresenceTracker`), pas de table dédiée — compteur volatile par nature, une seule instance en déploiement (`docker compose`). **Implémenté** (issue #15). | Contrat des 5 opérations intact                                                                                                         |
...
2. `docs/diagrammes/` : **D1** cas d'utilisation, **D2** classes/modèle (≡ migrations), **D3** séquence « marquer sa présence » (≡ codes HTTP), **D4** états-transitions de l'exercice _(bonus)_ — tous en Mermaid texte.
...
| 2       | _étape 3_  | **Prévu :** mise à jour après ouverture de l'enveloppe (évolution du nombre de relecteurs) — un cahier des charges périmé est un cahier des charges mort. |
